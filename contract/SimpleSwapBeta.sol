...
contract SimpleSwap is ERC1155withAdapter, Admin {
    ...
    function USDXToTokenInput(address token, uint256 USDX_sold, uint256 min_tokens, uint256 deadline, address buyer, address recipient) private returns (uint256) {
        ...
        uint256 tokens_bought = getInputPrice(USDX_sold, USDXReserveOf[token], TokenReserveOf[token]);
        USDXReserveOf[token] = USDXReserveOf[token].add(USDX_sold);
        TokenReserveOf[token] = TokenReserveOf[token].sub(tokens_bought);
    ...
    function USDXToTokenOutput(address token, uint256 tokens_bought, uint256 max_USDX, uint256 deadline, address  buyer, address recipient) private returns (uint256) {
        ...
        uint256 USDX_sold = getOutputPrice(tokens_bought, USDXReserveOf[token], TokenReserveOf[token]);
        USDXReserveOf[token] = USDXReserveOf[token].add(USDX_sold);
        TokenReserveOf[token] = TokenReserveOf[token].sub(tokens_bought);
    ...
    function tokenToUSDXInput(address token, uint256 tokens_sold, uint256 min_USDX, uint256 deadline, address buyer, address recipient) private returns (uint256) {
        ...
        uint256 USDX_bought = getInputPrice(tokens_sold, TokenReserveOf[token], USDXReserveOf[token]);
        USDXReserveOf[token] = USDXReserveOf[token].sub(USDX_bought);
        TokenReserveOf[token] = TokenReserveOf[token].add(tokens_sold);
    ...
    function tokenToUSDXOutput(address token, uint256 USDX_bought, uint256 max_tokens, uint256 deadline, address buyer, address recipient) private returns (uint256) {
        ...
        uint256 tokens_sold = getOutputPrice(USDX_bought, TokenReserveOf[token], USDXReserveOf[token]);
        USDXReserveOf[token] = USDXReserveOf[token].sub(USDX_bought);
        TokenReserveOf[token] = TokenReserveOf[token].add(tokens_sold);
        ...
        private returns (uint256)
    {
        ...
        uint256 USDX_bought = getInputPrice(tokens_sold, TokenReserveOf[input_token], USDXReserveOf[input_token]);

        uint256 token_bought = getInputPrice(USDX_bought, USDXReserveOf[output_token], TokenReserveOf[output_token]);
        ...
        TokenReserveOf[output_token] = TokenReserveOf[output_token].sub(token_bought);
        ...
        private returns (uint256)
    {
        ...
        uint256 USDX_bought = getOutputPrice(tokens_bought, USDXReserveOf[output_token], TokenReserveOf[output_token]);
        ...
        TokenReserveOf[output_token] = TokenReserveOf[output_token].sub(tokens_bought);
        TokenReserveOf[input_token] = TokenReserveOf[input_token].add(tokens_sold);
    ...
    function tokenToTokenOutputHelper(address input_token, uint256 USDX_bought) private view returns(uint256) {
        uint256 tokens_sold = getOutputPrice(USDX_bought, TokenReserveOf[input_token], USDXReserveOf[input_token]);
        return  tokens_sold;
    }
    ...
    function getUSDXToTokenInputPrice(address token, uint256 usdx_sold) public view returns (uint256) {
        require(usdx_sold > 0);
        return getInputPrice(usdx_sold, USDXReserveOf[token], TokenReserveOf[token]);
    }
    ...
    function getUSDXToTokenOutputPrice(address token, uint256 tokens_bought) public view returns (uint256) {
        ...
        return getOutputPrice(tokens_bought, USDXReserveOf[token], TokenReserveOf[token]);
    }
    ...
    function getTokenToUSDXInputPrice(address token, uint256 tokens_sold) public view returns (uint256) {
        ...
        return getInputPrice(tokens_sold, TokenReserveOf[token], USDXReserveOf[token]);
    }
    ...
    function getTokenToUSDXOutputPrice(address token, uint256 usdx_bought) public view returns (uint256) {
        ...
        return getOutputPrice(usdx_bought, TokenReserveOf[token], USDXReserveOf[token]);
    }
    ...
    function addLiquidity(address token, uint256 reserve_added, uint256 min_liquidity, uint256 max_tokens, uint256 deadline) public payable returns (uint256) {
        ...
        if (total_liquidity > 0) {
            ...
            uint256 token_amount = (reserve_added.mul(TokenReserveOf[token]) / USDXReserveOf[token]).add(1);
            ...
            TokenReserveOf[token] = TokenReserveOf[token].add(token_amount);
    ...
    function removeLiquidity(address token, uint256 amount, uint256 min_USDX, uint256 min_tokens, uint256 deadline) public returns (uint256, uint256) {
        ...
        TokenReserveOf[token] = TokenReserveOf[token].sub(token_amount);
